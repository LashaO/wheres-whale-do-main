...
class Config:
    ...
    crop_test_dir = (
        data_dir + "/test_images_yolo_x6_public_ensemble_crop_640/"
    )  # images 27956
...
class CustomPLModel(pl.LightningModule):
    ...
    def get_scheduler(self, optimizer):
        if self.CFG.scheduler == "ReduceLROnPlateau":
            ...
            scheduler = CosineAnnealingLR(
                optimizer, T_max=self.CFG.T_max, eta_min=self.CFG.min_lr, last_epoch=-1
            )
            ...
            scheduler_cosine = CosineAnnealingLR(
                optimizer, self.CFG.cosine_epochs, eta_min=1e-7
            )
    ...
    def mixup_criterion(self, pred, t_a, t_b, lam):
        return lam * self.train_criterion_p5(pred, t_a) + (
            1 - lam
        ) * self.train_criterion_p5(pred, t_b)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if CFG.do_mixup:
            if batch_idx == 0:
                LOGGER.info(
                    f"Training with mixup prob={self.CFG.mixup_prob} in epoch {self.current_epoch}"
                )
            images, individual_ids_a, individual_ids_b, mixup_lam = self.mixup_data(
                images, individual_ids, alpha=CFG.mixup_alpha
            )
        ...
        if CFG.do_cutmix:
            ...
            images, individual_ids = cutmix(
                images, individual_ids, alpha=1.0, prob=CFG.cutmix_prob
            )
        ...
        if batch_idx < 3 and self.current_epoch == 0:
            if os.path.exists(CFG.save_dir):
                save_img(
                    images,
                    CFG.save_dir + f"/train_{self.current_epoch}_{batch_idx}.png",
                    CFG.save_row_num,
                )
        ...
        if CFG.do_mixup:
            loss_p5 = self.mixup_criterion(
                logits_p5, individual_ids_a, individual_ids_b, mixup_lam
            )
        else:
        ...
        self.log(
            "loss_p5", loss_p5, on_step=True, on_epoch=True, prog_bar=True, logger=True
        )
        self.log("loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)

        logger_logs = {"loss": loss, "lr": lr}
        output = {"loss": loss, "progress_bar": logger_logs, "log": logger_logs}
        return output
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx < 3 and self.current_epoch == 0:
            if os.path.exists(CFG.save_dir):
                save_img(
                    images,
                    CFG.save_dir + f"/valid_{self.current_epoch}_{batch_idx}.png",
                    CFG.save_row_num,
                )
    ...
    def validation_epoch_end(self, outputs):
        ...
        #         self.log('map5', map5)
        LOGGER.info(
            f"Epoch={self.current_epoch},avg_val_loss={avg_val_loss:06f}, mAP5={mAP5:06f}"
        )
...
class CustomDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.CFG.debug:
            df_fold = df_fold.sample(
                n=self.CFG.batch_size * 10, random_state=self.CFG.seed
        ...
        if self.CFG.do_resample:
            ...
            df_number_1 = pd.DataFrame(
                np.repeat(df_number_1.values, 5, axis=0), columns=df_number_1.columns
            )

            ### number == 2
            df_number_2 = df_train[df_train.number == 2]
            df_number_2 = pd.DataFrame(
                np.repeat(df_number_2.values, 4, axis=0), columns=df_number_2.columns
            )

            ### number == 3
            df_number_3 = df_train[df_train.number == 3]
            df_number_3 = pd.DataFrame(
                np.repeat(df_number_3.values, 3, axis=0), columns=df_number_3.columns
            )

            ### number == 4
            df_number_4 = df_train[df_train.number == 4]
            df_number_4 = pd.DataFrame(
                np.repeat(df_number_3.values, 2, axis=0), columns=df_number_4.columns
            )

            df_train = pd.concat(
                [df_number_large, df_number_1, df_number_2, df_number_3, df_number_4]
            )
            print(f"df train shape is {df_train.shape}")

        self.train_dataset = TrainMultiHeadDataset(
            CFG, df_train, transforms=get_train_transforms(self.CFG)
        )

        self.valid_dataset = TrainMultiHeadDataset(
            CFG, df_valid, transforms=get_val_transforms(self.CFG)
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train_dataset, self.CFG.batch_size, num_workers=4, shuffle=True
        )

    def val_dataloader(self):
        return DataLoader(
            self.valid_dataset, self.CFG.batch_size * 2, num_workers=4, shuffle=False
        )
...
def train_loop(CFG, fold, LOGGER):
    ...
    LOGGER.info(
        f"Training model {CFG.model_name}, params with batch_size={CFG.batch_size}, image_size={CFG.image_size}, embedding_size={CFG.embedding_size}, scheduler={CFG.scheduler}, init_lr={CFG.lr}, fl_gamma={CFG.fl_gamma}, s={CFG.s},m={CFG.m},s_p4={CFG.s_p4},m_p4={CFG.m_p4},s_p3={CFG.s_p3},m_p3={CFG.m_p3}, warmup_epochs={CFG.warmup_epochs}, cosine_epochs={CFG.cosine_epochs}."
    )
    ...
    tb_logger = TensorBoardLogger(
        save_dir=CFG.save_dir, name=f"{CFG.model_name}", version=f"fold_{fold}"
    )
